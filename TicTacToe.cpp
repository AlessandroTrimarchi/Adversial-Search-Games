...
int main(){
    ...
    printf("Thaks for playing!");

    return 0;
}

void resetBoard(){
    //nested loops to inizialize an empty board
    for(int i = 0; i < 3; i++){
        for(int j = 0; j < 3; j++){
            board[i][j] = ' ';
        }
...
char checkWinner(){
    ...
    //check columns
    for(int i = 0; i < 3; i++){
        if(board[0][i] == board[1][i] && board[0][i] == board[2][i]) return board[i][0];
    }
    //check diagonals
    if(board[0][0] == board[1][1] && board[0][0] == board[2][2]) return board[0][0];
    if(board[0][2] == board[1][1] && board[0][2] == board[2][0]) return board[2][0];
...
void printWinner(char w){
    if(w == PLAYER) printf("YOU WIN!");
    else if(w == COMPUTER) printf("YOU LOSE!");
